...
import 'package:just_audio_platform_interface_custom/just_audio_platform_interface_custom.dart';
...
Future<Directory> _getCacheDir() async =>
    Directory(p.join((await getTemporaryDirectory()).path, 'just_audio_custom_cache'));
...
class AndroidEqualizerBand {
  ...
  Future<void> setGainWithIndex(int indexEq, double gain) async {
    _gainSubject.add(gain);
    if (_player._active) {
      await (await _player._platform).androidEqualizerBandSetGain(
          AndroidEqualizerBandSetGainRequest(bandIndex: indexEq, gain: gain));
    }
  }
